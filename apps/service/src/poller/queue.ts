        ...
        await Promise.all(
          tasks.map(async (task) => {
            ...
            const parsedPayload = JSON.parse(
              task.VisitEvent.payload?.toString() || "{}",
            );
            ...
            const payload: RegistrationPayload = {
              ...
              tanggalperiksa: task.VisitEvent.tanggal
                .toISOString()
                .split("T")[0],
                ...
                await prisma.$transaction(async (tx) => {
                  ...
                  await tx.visitEventLog.create({
                    data: {
                      ...
                      last_error:
                        response.data?.metadata?.message || "Unknown Error",
                      sentAt: new Date(),
                    },
                  });
                });
      ...
      if (tasks.length > 0) {
        await Promise.all(
          tasks.map(async (task) => {
            ...
            try {
              const response = await sendToBpjs(
                "/antrean/updatewaktu",
                payload,
              );
              const responseCode = response.data?.metadata?.code;
